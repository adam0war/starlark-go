...
func unquote(quoted string) (s string, triple, isByte bool, err error) {
	...
	for {
		...
		case '0', '1', '2', '3', '4', '5', '6', '7':
			...
			if !isByte && n > 127 {
				err = fmt.Errorf(`non-ASCII octal escape \%o (use \u%04X for the UTF-8 encoding of U+%04X, or non-portable \X%02X for a raw byte)`, n, n, n, n)
				return
			}
			if n >= 256 {
		...
		case 'x', 'X':
			...
			// \X variant (in text strings only) denotes a raw UTF-8 code unit,
			// but is non-portable: Java's \X requires four hex digits and
			// encodes a raw UTF-16 code unit.
			if isByte && quoted[1] == 'X' {
				err = fmt.Errorf(`\X escape in byte string (use \x)`)
				return
			}
			if !isByte && n > 127 && quoted[1] == 'x' {
				err = fmt.Errorf(`non-ASCII hex escape %s (use \u%04X for the UTF-8 encoding of U+%04X, or non-portable \X%02X for a raw byte)`,
					quoted[:4], n, n, n)
				return
			}
			buf.WriteByte(byte(n))
